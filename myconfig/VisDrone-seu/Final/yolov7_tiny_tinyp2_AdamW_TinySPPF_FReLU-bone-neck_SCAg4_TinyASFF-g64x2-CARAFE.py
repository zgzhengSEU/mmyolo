...
# ======================== wandb & run ==============================
TAGS = ["SEU", "load", "tinyp2","AdamW", 'SCA', 'TinyASFF', 'CARAFE', "FReLU", "TinySPPF"]
GROUP_NAME = "yolov7_tiny-final"
ALGO_NAME = "yolov7_tiny_tinyp2_AdamW_TinySPPF_FReLU-bone-neck_SCAg4_TinyASFF-g64x2-CARAFE"
...
backbone_FReLU=True
neck_FReLU=True
use_carafe_neck=True
use_carafe_asff=True
model = dict(
    backbone=dict(
        plugins=[
            dict(
    ...
    neck=[
        dict(
            ...
            block_cfg=dict(type='TinyDownSampleBlock', middle_ratio=0.25, use_FReLU=neck_FReLU),
            act_cfg=dict(type='LeakyReLU', negative_slope=0.1),
        ...
        dict(
            ...
            widen_factor=0.5,
            head_num=4,
            use_carafe=use_carafe_asff,
            groups=64,
            use_two_group_expand=True,
            use_att='TinyASFF')],
    bbox_head=dict(
        head_module=dict(
            in_channels = [64, 128, 256, 512],
